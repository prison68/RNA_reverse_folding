...
class RNAFeatures(nn.Module):
    def __init__(self, edge_features, node_features, node_feat_types=[], edge_feat_types=[], num_rbf=16, top_k=30, augment_eps=0.2, dropout=0.1, args=None):
        super(RNAFeatures, self).__init__()
        """生成RNA特征"""
        self.edge_features = edge_features
        self.node_features = node_features
        self.top_k = top_k
        self.augment_eps = augment_eps # 0.2
    ...
    def forward(self, X, S, mask):
        ...
        # 把原始的骨架三维坐标作为节点特征
        x_node = node_mask_select(X.view(B, N, -1))
        h_V.append(x_node)

        # 放到一个列表里
        if 'angle' in self.node_feat_types:
            h_V.append(V_angle)     # 12
        if 'distance' in self.node_feat_types:
            h_V.append(V_dist)      # 9
        if 'direction' in self.node_feat_types:
            h_V.append(V_direct)    # 80

        if 'orientation' in self.edge_feat_types:
            h_E.append(E_orient)     # 4
        if 'distance' in self.edge_feat_types:
            h_E.append(E_dist)       # 96
        if 'direction' in self.edge_feat_types:
            h_E.append(E_direct)     # 15

        # 特征编码
        h_V = self.norm_nodes(self.node_embedding(torch.cat(h_V, dim=-1)))      # 在最后一个维度上concat起来，在进行线性层编码得到指定的特征维度128
        h_E = self.norm_edges(self.edge_embedding(torch.cat(h_E, dim=-1)))      # 也是一样的，128
