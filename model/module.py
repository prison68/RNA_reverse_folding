...
class MPNNLayer(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        ...
        # transform放到mpnn的后面
        self.trans = TransformerLayer(num_hidden, num_hidden*2, num_heads=4, dropout=0.1)
    ...
    def forward(self, h_V, h_E, edge_idx, batch_id=None):
        ...
        # transformer层
        h_V = self.trans(h_V, h_E, edge_idx, batch_id)
        return h_V
...
class NeighborAttention(nn.Module):
    ...
    def forward(self, h_V, h_E, center_id, batch_id):
        ...
        h_V = scatter_sum(attend * V, center_id, dim=0).view(
            [N, self.num_hidden])  # 再与节点特征相乘，使用scatter_sum聚合邻居边对中心节点的贡献
